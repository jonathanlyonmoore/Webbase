...
{
   char tmp_buffer[MAX_URL_SIZE];
   
   // check if the url is too long
   if (end - start > MAX_URL_SIZE - 10)  return -1;

   // parse the url
   if (parse(start, end, tmp_buffer, MAX_URL_SIZE) == NULL) {
      initialize();
      return -1;
   }
...
int dynamic_url::replace(const char* start, const char* end, const url& base)
{
   ...
   if (merge(base, path, MAX_URL_SIZE) == NULL) {
      initialize();
      return -1;
   }
   if (allocate_space() == NULL) {
...
static_url::static_url(const char* a, const char* b, const char* c, 
   ...
   : url(a, b, c, d, e, f, g)
{
   if (store(buffer, MAX_URL_SIZE) == NULL) {
      initialize();
   }
}
...
istream& operator >> (istream& is, dynamic_url& u)
{
   ...
   count = 0;
   while (count < url::MAX_URL_SIZE - 1 && ((c = is.get()) != EOF) &&
	  (c > ' ' && c != 0x7f)) {
      holder[count++] = c;
   }
   holder[count] = 0;
