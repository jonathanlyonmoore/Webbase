#!/usr/bin/perl -w
#
# loopprocess.pl - Iterate process(->handlers) over the Web repository
...
die "Usage: --configfile=configuration_filename [repository URI]\n"
...
$errorpause = 1;  # Pause on errors (requiring user input)
...
$PROCESSCMD = &ConfLoader::getValue('PROCESS');
die "PROCESS not defined in $CONFIGFILE\n" unless $PROCESSCMD;

# $PROCESS = "$PROCESSCMD rep://${WEBCAT_REPOSITORY}";
$SOURCE = $ARGV[1];
$SOURCE = "rep://${WEBCAT_REPOSITORY}" unless $SOURCE;
$PROCESS = "$PROCESSCMD $CONFIGFILE $SOURCE";
...
for $iteration ( 1 .. $ITERATION_COUNT ) {
	...
	# $command = "$WEBCAT $CURRENT_OFFSET $WEBCAT_REPOSITORY | $MAKETABLESCMD"; 
        $command = "$PROCESS $ITERATION_LENGTH";
	open(INPUT, "$command |") ||
