...
#ifdef HAVE_SRB
   // FIXME Logic to figure out how to make an SRB repository goes here
   repo      = new repository(std::string(repName), true, 'a'); 
#else
   repo      = new repository(std::string(repName), true, (*disklist)[0], 'a'); 
#endif
#endif 
#endif
...
int manager::request_crawl(const std::string& request)
{
   int  entry_num;
...
int manager::suspend_crawl(int queue_id, std::ostream& os)
{
   int  rc;
   int  entry_num;
...
#ifndef JUN_ARCHIVER
   // hint to repository that the site is complete (at least for now)
   repo->end_site(entry.site_names[0], entry.success_count);
#endif
 ...
 queue_select_loop:
   ...
   switch (rc) {
	 ...
	 // send report
	 {
	    ...
	    // <wlam@cs.stanford.edu> Jun 2003
	    writemsg(reportstr, "Fetched", itoa(entry.success_count));
	    // <wlam@cs.stanford.edu> Jun 2003
	    reportstr << endl;
...
#ifndef JUN_ARCHIVER
            // hint to repository that the site is complete
            repo->end_site(entry.site_names[0], entry.success_count);
#endif
...
int manager::load_queue(const std::string& packet)
{	    
   int  entry_num, rc;
...
int manager::queue_info::save(std::ostream& os, const char* name)
{
   os << name << "Start" << endl;
...
int manager::queue_info::load(std::istream& is, const char* name)
{
