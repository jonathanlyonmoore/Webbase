...
class manager {
   public:
      ...
      int   request_crawl(const std::string& packet);
      int   load_queue(const std::string& packet);
      int   suspend_crawl(int queue_id, std::ostream& os);
      int   main_loop();
   ...
   private:
      struct queue_info {
	    ...
	    std::vector<unsigned>        site_ips;
	    url_queue                    queue;
	    url_filter                   filter;

	    std::vector<int>             url_count_per_seed;
	    ...
	    int reset();
	    int save(std::ostream& out, const char* name = "QueueInfo");
	    int load(std::istream& in,  const char* name = "QueueInfo");
      };
      struct request_info {
	    int      request_id;
	    int      seed_id;
