...
// the maximum number of characters past the path of a URL that
// should be kept in the log entry indicating the fetch of that URL
static const int MAX_LOG_QUERY_LEN = 100;
// The number should be greater than zero so that the log will clearly
// distinguish between different URLs querying the same resource with
// different parameters.  Larger numbers allow longer query strings
// into the log unabbreviated.
...
void crawl_callback(void* param, fetch_module::response* rsp)
{
 ...
 parse_page:
   if (VALID_PAGE(rsp->error_code, rsp->page_size) && 
      ...
      while ((tag = parser.get_link(url_start, url_end)) >= 0) {
         // The parser should interpret SGML character entities from HTML %URI;
         // attribute types, but it doesn't, so we do it here.
         // There "should" be a nice function to do the transformations,
         // but we are not supporting the entire library of entities yet.
         // (Only ampersand (&amp; -> &) is essential:  It is a valid URI
         // character, and it must be escaped as an SGML character entity 
         // in %URI; attributes in HTML4.)
         // Anybody serious about doing the rest of the SGML character entities
         // supported in HTML4 (HTML 4.01 Section 24) can factor out the code 
         // below into a separate function and do it right. -- wlam, 5 Sep 2003
         string URI_string(url_start, url_end);

         // (Transform other SGML entities here.)

         // Now sweep through URL_string and transform &amp; -> &.
         string SGML_amp("&amp;"), interpreted_amp("&");
         string::size_type pos = URI_string.find(SGML_amp);
         while (pos != string::npos) {
            URI_string.replace(pos, SGML_amp.length(), interpreted_amp);
            pos = URI_string.find(SGML_amp, pos+interpreted_amp.length());
         }
         const char *transformed = URI_string.data();
         url_start = transformed;  url_end = transformed+URI_string.length();
   ...
   if (VALID_PAGE(rsp->error_code, rsp->page_size) && req->seed_id == 0) {
      ...
      // a suboptimal solution to follow the renaming of the crawler:
      parse_robot_file(page, "WebVac", excl);
 ...
 store_page:
   ...
   if (VALID_PAGE(rsp->error_code, rsp->page_size) &&
       ...
       const int   BUFFER_SIZE = CRAWLER_MAX_URL_SIZE+1;
...
#else
       if ((rc = req->mg->repo->store(queue_entry.site_names[0], string(url),
                                      rsp->page, rsp->page_size)) < 0) {
#endif
...
#if defined(HISTORY)
   {
      ...
      const int   URL_BUFFER_SIZE = CRAWLER_MAX_URL_SIZE+1;
  ...
  history_exit:
   ...
   // Let's keep a little query information in the log, so different queries
   // of the same script will look obviously different. --wlam, 12 Aug 2003
   {
   std::ostringstream url_stream;
   // Reconstruct the rest of the URL after the path, the way url.cc's
   // ostream& operator<<(ostream&, const url&) does it.
   if (strlen(rsp->page_url->param) > 0)
      url_stream << ';' << rsp->page_url->param;
   if (strlen(rsp->page_url->query) > 0)
      url_stream << '?' << rsp->page_url->query;
   // Length-limit the resulting string, to keep the log lines short.
   int len;
   if ((len=url_stream.str().length()) > MAX_LOG_QUERY_LEN)
      // The expression '<...>' is chosen to indicate that url_stream is
      // abbreviated in the log message to stay within MAX_LOG_QUERY_LEN.
      // The '<' and '>' characters are disallowed from URIs (RFC 2396),
      // so the probability of misinterpreting this expression should be slim.
   {
      LOG(("Done: %s%s<...>%s (id:%d seed:%d level:%d status:%d size:%d %d ms)",
           rsp->page_url->net_loc, rsp->page_url->path,
           &url_stream.str().c_str()[len - MAX_LOG_QUERY_LEN],
