...
int main(int argc, const char* argv[])
{
  ...
  //Pranav end
   ...
   string strTypesParse, strTypesStore;
   string strFollowIMG;
   ...
   strExplanation = "Types (follow links)";
   crawlparam.CreateRecord(&strTypesParse, CRAWLSERVER, "TypesParse", "p", strExplanation);

   strExplanation = "Types (store)";
   crawlparam.CreateRecord(&strTypesStore, CRAWLSERVER, "TypesStore", "t", strExplanation);
   ...
   split_string(strTypesParse, ",", str_list);
   url_filter::set_parse_types(str_list);

   split_string(strTypesStore, ",", str_list);
   url_filter::set_store_types(str_list);
...
#ifdef SOCKET_CONTROL
   if (daemonize) daemon(1 /* don't chdir */ ,1);
#endif
