...
const int BUFFER_SIZE = CRAWLER_MAX_PAGE_SIZE;
...
int main(int argc, char* argv[])
{
    ...
    repo = new repository(repository::BIGFILE_REP,string(repName));
    ...
    while ((rc = repo->read(url, time, buffer, bufSize)) >= 0) {
       ...
       process(url.c_str(), time, pos, buffer, buffer + bufSize);
       if (++ count % 10000 == 0) {
	    ...
	    const char* start, char* end)
{
   ...
   static char url_buffer[CRAWLER_MAX_URL_SIZE+1];
   ...
   while ((tag = parser.get_link(url_start, url_end)) >= 0) {
	 ...
	 if (tag == html_parser::AREA || tag == html_parser::A ||
	    ...
	    link.strcpy(url_buffer, url_buffer + CRAWLER_MAX_URL_SIZE);
	    printf("%s\n", url_buffer);
	 }
      }
   }
