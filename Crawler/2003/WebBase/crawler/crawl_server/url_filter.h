...
// FIXME (I'll get rid of this soon)
using namespace std;
...
private: 
      ...
      // create a set for each action, to keep track of which Types
      // are triggers for that action
      static set<TypeDetector::Type> types_parse;
      static set<TypeDetector::Type> types_store;

      static bool f_img_enabled;

      static TypeDetector typeDetector;

      static ostream& error() { return (cerr << "Error in url_filter: "); }
...
public:
      ...
      static void set_store_types(const vector<string>& type_names) {
	load_types(types_store, type_names);
      }
      static void set_parse_types(const vector<string>& type_names) {
	load_types(types_parse, type_names);
      }
      static void load_types(set<TypeDetector::Type>& types, 
			     const vector<string>& type_names);

      static bool is_enabled(set<TypeDetector::Type>& types_set, 
			     const url* url);
      static bool parse_enabled(const url* url);
      static bool store_enabled(const url* url);

      /** are urls from the <img> tag allowed */
      static bool img_enabled() { return f_img_enabled; }
      static void set_img_enabled(bool f) { f_img_enabled = f; }
};

inline ostream& operator << (ostream& os, 
                                  const url_filter::seed_entry& se)
{
