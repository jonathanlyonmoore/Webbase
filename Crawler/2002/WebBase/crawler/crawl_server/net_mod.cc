...
int net_mod::resolve_name(const string& site_name, 
			  vector<unsigned>& ip_list, 
...
unsigned net_mod::inet_addr(const string& ip_str)
{
   union {
...
const string net_mod::inet_ntoa(unsigned my_ip)
{
   string result;
   union {
	 unsigned char digit[4];
