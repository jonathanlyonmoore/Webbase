...
class url_queue {
   public:
      ...
      int              reset();
      int              save(ostream& out, const char* name = "UrlQueue");
      int              load(istream& in,  const char* name = "UrlQueue");

   private:
      // url queue and url table
      int                   seen_count;
      bit_vector           *urls_seen;
      deque<url_info>       urls_to_crawl;

      unsigned hash_url(const url* u) const;
};

inline ostream& operator << (ostream& os, const url_queue::url_info& ui)
{
   os << *ui.u << ' ' << ui.seed_id << ' ' << ui.level;
   return os;
}

inline istream& operator >> (istream& is, url_queue::url_info& ui)
{
